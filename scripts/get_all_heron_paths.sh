...
function get_heron_thirdparty_dependencies() {
  ...
  echo "$(find {bazel-bin/sql,bazel-bin/heron/proto,bazel-genfiles/external,bazel-heron/bazel-out/host/bin/third_party,bazel-heron/bazel-out/host/genfiles/external}/. -name "*jar" -type f | sort -u)";
}
